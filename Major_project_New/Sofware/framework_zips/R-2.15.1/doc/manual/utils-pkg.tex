...
download methods \code{"internal"}, \code{"wget"}, \code{"curl"}
and \code{"lynx"} are available, and there is a value \code{"auto"}: see
...
(e.g. \samp{https://} URLs or those than use cookies) which the
...
CMD INSTALL} is used.  To use only that sub-architecture, use
...
The functions described here work under the \code{readline}
...
Note that \code{readline} history library saves files with permission
